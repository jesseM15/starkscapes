	...
	<section id="admin_services_service" class="admin-section">
		<div class="admin_wrap container-fluid">
			...
			<?php foreach ($images as $image) : ?>
				<div id="gallery-images" class="col-sm-6 col-md-3 <?= $this->uri->segment(3) . '_' . $image['id'] . '_' . $image['rank'] ?>">
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<form id="gallery-form" method="post" action="<?= base_url() ?>admin_gallery/category/<?= $this->uri->segment(3) . '/' . $this->uri->segment(4) ?>" enctype="multipart/form-data">
							<label class="upload-button btn btn-lg btn-success" for="gallery-file">
								Upload Image
								<input id="gallery-file" type="file" name="file" style="display:none;" form="gallery-form" onchange="this.form.submit();">
							</label>
							<input type="hidden" id="selectedImage" name="selectedImage" />
							<button type="button" class="gallery-select-button btn btn-lg btn-default" data-dismiss="modal">Select</button>
						</form>
					</div>
				</div>
			</div>
